    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.comments.update", [$comment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('body') ? 'is-invalid' : '' }}" name="body" id="body">{{ old('body', $comment->body) }}</textarea>
                @if($errors->has('body'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('commentable') ? 'is-invalid' : '' }}" name="commentable_id" id="commentable_id" required>
                    @foreach($commentables as $id => $entry)
                        <option value="{{ $id }}" {{ (old('commentable_id') ? old('commentable_id') : $comment->commentable->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('commentable_type') ? 'is-invalid' : '' }}" type="text" name="commentable_type" id="commentable_type" value="{{ old('commentable_type', $comment->commentable_type) }}" required>
