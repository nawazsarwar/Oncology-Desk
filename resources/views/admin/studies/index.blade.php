@extends('layouts.admin')
@section('content')
@can('study_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.studies.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.study.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.study.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-Study">
            <thead>
                <tr>
                    <th width="10">

                    </th>
                    <th>
                        {{ trans('cruds.study.fields.id') }}
                    </th>
                    <th>
                        {{ trans('cruds.study.fields.modality') }}
                    </th>
                    <th>
                        {{ trans('cruds.study.fields.type') }}
                    </th>
                    <th>
                        {{ trans('cruds.study.fields.name') }}
                    </th>
                    <th>
                        {{ trans('cruds.study.fields.fee') }}
                    </th>
                    <th>
                        {{ trans('cruds.study.fields.maximum_slots') }}
                    </th>
                    <th>
                        {{ trans('cruds.study.fields.time_per_slot') }}
                    </th>
                    <th>
                        {{ trans('cruds.study.fields.films') }}
                    </th>
                    <th>
                        {{ trans('cruds.study.fields.weightage') }}
                    </th>
                    <th>
                        {{ trans('cruds.study.fields.facility') }}
                    </th>
                    <th>
                        {{ trans('cruds.study.fields.status') }}
                    </th>
                    <th>
                        {{ trans('cruds.study.fields.remarks') }}
                    </th>
                    <th>
                        &nbsp;
                    </th>
                </tr>
            </thead>
        </table>
    </div>
</div>



@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('study_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.studies.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).data(), function (entry) {
          return entry.id
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  let dtOverrideGlobals = {
    buttons: dtButtons,
    processing: true,
    serverSide: true,
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.studies.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'modality_name', name: 'modality.name' },
{ data: 'type', name: 'type' },
{ data: 'name', name: 'name' },
{ data: 'fee', name: 'fee' },
{ data: 'maximum_slots', name: 'maximum_slots' },
{ data: 'time_per_slot', name: 'time_per_slot' },
{ data: 'films', name: 'films' },
{ data: 'weightage', name: 'weightage' },
{ data: 'facility_name', name: 'facility.name' },
{ data: 'status', name: 'status' },
{ data: 'remarks', name: 'remarks' },
{ data: 'actions', name: '{{ trans('global.actions') }}' }
    ],
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  };
  let table = $('.datatable-Study').DataTable(dtOverrideGlobals);
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
});

</script>
@endsection