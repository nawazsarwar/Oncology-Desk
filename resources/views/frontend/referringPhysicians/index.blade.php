@extends('layouts.frontend')
@section('content')
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            @can('referring_physician_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.referring-physicians.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.referringPhysician.title_singular') }}
                        </a>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                            {{ trans('global.app_csvImport') }}
                        </button>
                        @include('csvImport.modal', ['model' => 'ReferringPhysician', 'route' => 'admin.referring-physicians.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                <div class="card-header">
                    {{ trans('cruds.referringPhysician.title_singular') }} {{ trans('global.list') }}
                </div>

                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-ReferringPhysician">
                            <thead>
                                <tr>
                                    <th>
                                        {{ trans('cruds.referringPhysician.fields.id') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.referringPhysician.fields.name') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.referringPhysician.fields.gender') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.referringPhysician.fields.mobile_no') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.referringPhysician.fields.email') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.referringPhysician.fields.department') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.referringPhysician.fields.address') }}
                                    </th>
                                    <th>
                                        &nbsp;
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($referringPhysicians as $key => $referringPhysician)
                                    <tr data-entry-id="{{ $referringPhysician->id }}">
                                        <td>
                                            {{ $referringPhysician->id ?? '' }}
                                        </td>
                                        <td>
                                            {{ $referringPhysician->name ?? '' }}
                                        </td>
                                        <td>
                                            {{ App\Models\ReferringPhysician::GENDER_SELECT[$referringPhysician->gender] ?? '' }}
                                        </td>
                                        <td>
                                            {{ $referringPhysician->mobile_no ?? '' }}
                                        </td>
                                        <td>
                                            {{ $referringPhysician->email ?? '' }}
                                        </td>
                                        <td>
                                            {{ $referringPhysician->department ?? '' }}
                                        </td>
                                        <td>
                                            {{ $referringPhysician->address ?? '' }}
                                        </td>
                                        <td>
                                            @can('referring_physician_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.referring-physicians.show', $referringPhysician->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('referring_physician_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.referring-physicians.edit', $referringPhysician->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('referring_physician_delete')
                                                <form action="{{ route('frontend.referring-physicians.destroy', $referringPhysician->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                                    <input type="hidden" name="_method" value="DELETE">
                                                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                    <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                                </form>
                                            @endcan

                                        </td>

                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>

        </div>
    </div>
</div>
@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('referring_physician_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('frontend.referring-physicians.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
          return $(entry).data('entry-id')
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  $.extend(true, $.fn.dataTable.defaults, {
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  });
  let table = $('.datatable-ReferringPhysician:not(.ajaxTable)').DataTable({ buttons: dtButtons })
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
})

</script>
@endsection